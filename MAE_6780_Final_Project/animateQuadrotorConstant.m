...
function animateQuadrotorConstant(state,XDes,filename,animate)
% Unpack state object
t   = state.Time;
x      = state.Data(:,1);
% dx     = state.Data(:,2);
...
% CMtrack = line('xdata',0,'ydata',0,'zdata',0,'color','black','marker',...
%     'LineWidth',2,'r-','MarkerSize',10);

%% Plot figures on right side
subplot(4,4,[3 4])
hold on
plot(t,phi,'k','LineWidth',1)
plot(tdes,phides*ones(length(tdes),1),'r--','LineWidth',1)
hold off
% title('$\phi$');
xlabel('t (s)');
ylabel('$\phi$ (rad)');
...
plot(tdes,thetades*ones(length(tdes),1),'r--','LineWidth',1)
hold off
% title('$\theta$');
xlabel('t (s)');
ylabel('$\theta$ (rad)');
...
plot(tdes,psides*ones(length(tdes),1),'r--','LineWidth',1)
hold off
% title('$\psi$');
xlabel('t (s)');
ylabel('$\psi$ (rad)');
...
plot(tdes,zdes*ones(length(tdes),1),'r--','LineWidth',1)
hold off
xlabel('t (s)')
ylabel('z (rad)')
...
scale = 2; % Longer scale means slower animation

%% Set up saving animation
...
%     set(CMtrack,'xdata',xval,'ydata',0,'zdata',0,'color','black','marker',...
%     'LineWidth',2,'r-','MarkerSize',10);
